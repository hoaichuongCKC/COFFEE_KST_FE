...
class DeliveringOrderView extends StatelessWidget {
  const DeliveringOrderView({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<MyInvoiceCubit, MyInvoiceState>(
      ...
      builder: (context, state) {
        ...
        if (state.listDelivering.isEmpty) {
          return Center(
            child: TextWidgets(
        ...
        if (state.stateApi is SuccessState) {
          return ListView.separated(
            ...
            itemCount: state.listDelivering.length,
            itemBuilder: (BuildContext context, int index) {
              return ItemInvoice(
                invoiceModels: state.listDelivering[index],
                index: index,
